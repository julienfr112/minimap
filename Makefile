...
build:
	...
	@for b in minimap minimap-backend anon-bake anon-serve; do \
	    printf "  %-16s %s\n" "$$b" "$$(du -h target/release/$$b | cut -f1)"; done
...
targets:
	...
	@echo
	@echo 'inputs -- expensive; "make clean" keeps them, "make clean-pbf" removes them' 
...
help:
	...
	@others="$$(ls -1 $(PMTILES)/*.pmtiles 2>/dev/null | sed 's|.*/||; s|\..*||' | sort -u | grep -vx '$(NAME)' | tr '\n' ' ')"; \
